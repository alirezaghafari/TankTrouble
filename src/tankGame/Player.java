...
/**
 * the class which contains the player properties
 */
public class Player {
    ...
    private void move(double speed) {
        ...
        if (cornerCrash(nextPoint, GameEngine.tankWidth)) {//if the tank would crash into a corner...
            nextPoint = new Point(this.coordinates.getXCoordinate()//try moving it horizontally
                    + (speed * Math.cos(Math.toRadians(90 - this.direction))),
                    this.coordinates.getYCoordinate());
            if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth) || wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
                nextPoint = new Point(this.coordinates.getXCoordinate(),//if moving it horizontally would cause a crash, try moving it vertically
                        this.coordinates.getYCoordinate()
                                - (speed * Math.sin(Math.toRadians(90 - this.direction))));
                if (cornerCrash(nextPoint, GameEngine.tankWidth) || wallCrashVertical(nextPoint, GameEngine.tankWidth) || wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
                    nextPoint = new Point(this.coordinates.getXCoordinate(),//if moving it vertically would also cause a crash, just don't move it
                            this.coordinates.getYCoordinate());
                }
            }
        } else {
            if (wallCrashVertical(nextPoint, GameEngine.tankWidth) && wallCrashHorizontal(nextPoint, GameEngine.tankWidth)) {
                nextPoint = new Point(this.coordinates.getXCoordinate(),//if moving it at all would cause a crash, just don't move it
                        this.coordinates.getYCoordinate());
            }
            if (wallCrashVertical(nextPoint, GameEngine.tankWidth)) { //if the tank would go over a vertical wall...

                nextPoint = new Point(this.coordinates.getXCoordinate(),//just move it vertically
                        this.coordinates.getYCoordinate()
                                - (speed * Math.sin(Math.toRadians(90 - this.direction))));
            }
    ...
    private boolean wallCrashVertical(Point point, int width) {
        ...
        boolean inLeftWall = (point.getXCoordinate() - width <=
        ...
        boolean inRightWall = (point.getXCoordinate() + width >=
    ...
    private boolean wallCrashHorizontal(Point p, int width) {
        //when given a point p, returns whether or not the circular object of width w at that point is in/over a wall
        width = width / 2;
        int yWall = 0;
        ...
        boolean inTopWall = (p.getYCoordinate() - width <=
        ...
        boolean inBottomWall = (p.getYCoordinate() + width >=
    ...
    private boolean cornerCrash(Point point, int width) {
        ...
        int xCounter = (int) point.getXCoordinate();
        ...
        int yCounter = (int) point.getYCoordinate();
        ...
        double distance1 = Point.distance(p1, point);
        double distance2 = Point.distance(p2, point);
        double distance3 = Point.distance(p3, point);
        double distance4 = Point.distance(p4, point);
    ...
    public void shoot() {
        if (this.numberOfBulletsFired < 5) {
            ...
            Point bulletStart = new Point(this.coordinates.getXCoordinate()
    ...
    public void hit() {
        ...
        ScoresPanel.reloadStamina(this.playerNumber, (int) stamina);
    }

    public double currentXSquare() {
        // return the x value of the grid square the centre of the tank is currently in
        for (int i = 0; i < 7; i++) {
            if ((GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * i < this.coordinates.getXCoordinate()) &&
                    (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.coordinates.getXCoordinate()) {
                return i;
            }
        }
        for (int i = 0; i < 8; i++) {
            if ((GameEngine.wallWidth * (i) + GameEngine.squareWidth * i <= this.coordinates.getXCoordinate()) &&
                    (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.coordinates.getXCoordinate()) {
                return i - 0.5;
            }
        }
    ...
    public double currentYSquare() {
        ...
        for (int i = 0; i < 7; i++) {
            if ((GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * i < this.coordinates.getYCoordinate()) &&
                    (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.coordinates.getYCoordinate()) {
                return i;
            }
        }
        for (int i = 0; i < 8; i++) {
            if ((GameEngine.wallWidth * (i) + GameEngine.squareWidth * i < this.coordinates.getYCoordinate()) &&
                    (GameEngine.wallWidth * (i + 1) + GameEngine.squareWidth * (i + 1)) > this.coordinates.getYCoordinate()) {
                return i - 0.5;
            }
        }
